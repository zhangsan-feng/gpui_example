...
fn main() {
    ...
    app.run(move |cx| {
        ...
        window_options.window_bounds = Some(WindowBounds::centered(size(px(400.), px(450.)), cx));
        window_options.is_resizable = false;
        window_options.titlebar = Some(TitlebarOptions{title: Some(SharedString::from("")), ..Default::default()});


        cx.open_window(window_options, |window, app| {
            gpui_component::init(app);
            state::new_state(app);
            let login_view = app.new(|cx| component::login::LoginView::new(window, cx));
